...
def pipeline(
    ...
    train_img_list = [
        c for idx, c in enumerate(img_list) if idx % llffhold != 0
    ]
    if n_views > 0:
        idx_sub = [
            round_python3(i)
            for i in np.linspace(0, len(train_img_list) - 1, n_views)
        ]
        train_img_list = [
            c for idx, c in enumerate(train_img_list) if idx in idx_sub
        ]
    ...
    with open("created/images.txt", "w") as fid:
        for idx, img_name in enumerate(img_rank):
            ...
            data = (
                [str(1 + idx)]
                + [" " + item for item in images[os.path.basename(img_name)]]
                + ["\n\n"]
            )
            fid.writelines(data)
...
for scene in [
    ...
    pipeline(
        scene, base_path="/ssd1/zehao/FSGS/dataset/mipnerf360/", n_views=24
