...
class GaussianModel:
    ...
    def create_from_pcd(
        ...
        print(
            "Number of points at initialisation : ", fused_point_cloud.shape[0]
        )
    ...
    def load_ply(
        ...
        self._rotation = nn.Parameter(
            torch.tensor(rots, dtype=torch.float, device="cuda").requires_grad_(
                True
            )
        )
    ...
    def densification_postfix(
        ...
        self.max_radii2D = torch.cat(
            [
                self.max_radii2D,
                torch.zeros(
                    self.get_xyz.shape[0] - self.max_radii2D.shape[0]
                ).to(self.max_radii2D),
            ]
        )
    ...
    def densify_and_clone(
        ...
        selected_pts_mask = torch.logical_and(
            selected_pts_mask,  # self.percent_dense = 0.01
            torch.max(self.get_scaling, dim=1).values
            <= self.percent_dense * scene_extent,
        )
