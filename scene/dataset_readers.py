...
def readColmapCameras(
    ...
    for idx, key in enumerate(sorted(cam_extrinsics.keys())):
        ...
        rgb_path = rgb_mapping[
            idx
        ]  # os.path.join(images_folder, rgb_mapping[idx])
        try:
...
def readColmapSceneInfo(
    ...
    rgb_mapping = [
        f
        for f in sorted(glob.glob(os.path.join(path, reading_dir, "*")))
        if f.endswith("JPG") or f.endswith("jpg") or f.endswith("png")
    ]
    cam_extrinsics = {
        cam_extrinsics[k].name: cam_extrinsics[k] for k in cam_extrinsics
    }
    ...
    if n_views > 0:
        ...
        train_cam_infos = [
            c for idx, c in enumerate(train_cam_infos) if idx in idx_sub
        ]
...
def readNerfSyntheticInfo(
    ...
    if eval:
        train_cam_infos = [
            c
            for idx, c in enumerate(train_cam_infos)
