...
def training(
    ...
    print_camera_list(
        train_cameras_all,
        save_image_dir=os.path.join(args.model_path, f"./drop_gaussian_train_camera_images/")
    )
    viewpoint_stack = None
    ema_loss_for_log = 0.0
    progress_bar = tqdm(
        range(first_iter, optimize_args.iterations),
    ...
    for iteration in range(first_iter, optimize_args.iterations + 1):
        ...
        bg = torch.rand(
            (3), device="cuda") if optimize_args.random_background else background
        render_pkg = render(
            viewpoint_cam, gaussians, pipeline_args, bg, is_train=True, iteration=iteration,
        ...
        with torch.no_grad():
            ...
            if iteration < optimize_args.densify_until_iter:
                ...
                if iteration > optimize_args.densify_from_iter and iteration % optimize_args.densification_interval == 0:
                    size_threshold = None
                    gaussians.densify_and_prune(
                        optimize_args.densify_grad_threshold, 0.005, scene.cameras_extent, size_threshold)

                if iteration % optimize_args.opacity_reset_interval == 0 or (dataset_args.white_background and iteration == optimize_args.densify_from_iter):
    ...
    if iteration in testing_iterations:
        ...
        for config in validation_configs:
            render_path = os.path.join(
                args.model_path, config['name'], "ours_{}".format(iteration), "renders")
            gts_path = os.path.join(
                args.model_path, config['name'], "ours_{}".format(iteration), "gt")
...
def build_parser():
    ...
    parser.add_argument(
        "--test_iterations", nargs="+",
        type=int, default=[5000, 10000,],
    )
    parser.add_argument(
        "--save_iterations", nargs="+",
    ...
    parser.add_argument(
        "--dropout_algorithm", type=str,
        default="largest",
        choices=["origin", "indices",],
    )
    return parser
